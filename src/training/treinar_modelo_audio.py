"""
Treino do classificador de indícios de fadiga/disartria na voz.

Rodar diretamente:  python -m src.training.treinar_modelo_audio

Mesmo "esqueleto" de treino dos outros dois modelos (ver
treinar_modelo_vitais.py para o racional completo de cada etapa).
"""
...
from src.training.conjunto_dados_audio import construir_conjunto_dados
from src.training.utils import avaliar_modelo, salvar_modelo_treinado

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(name)s: %(message)s")
logger = logging.getLogger("treino_audio")

NOME_MODELO = "audio_rf"


def treinar():
    logger.info("1/5 - Construindo conjunto de dados rotulado (características acústicas simuladas)...")
    ...
    print(f"\nModelo de áudio treinado e salvo em: {caminho}")
